...
export default function DetailPanel({ chemical }) {
  if (!chemical) {
    return (
      <aside className="detail-panel empty" aria-live="polite">
        <h2>Select a chemical</h2>
        <p>Tap any card to see full details and action guidance.</p>
      </aside>
    );
  }
  ...
  return (
    <aside className="detail-panel" aria-live="polite">
      <header>
        <h2>{chemical.name}</h2>
        <p>{chemical.category} • {chemical.unNumber}</p>
      </header>
